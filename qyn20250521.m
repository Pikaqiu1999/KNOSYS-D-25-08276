...
% results = table();
% sigma_values = [0.01, 0.03, 0.05];      % 高斯噪声标准差（作用于密文）
% clip_ratios = [0.01, 0.03, 0.05];        % 削波比例（百分比，作用于密文）
% for sigma = sigma_values
%     y_attacked = Y1 + sigma * randn(size(Y1));  % 攻击密文   
%     recovered_image = UserASPL(y_attacked, Phi, param, num_levels,R3);
%     errorx=sum(sum(abs(recovered_image - zy1).^2));   %MSE误差
%     psnr_val=10*log10(255*255/(errorx/high/width))   %  PSNR
%     ssim_val = ssim(recovered_image, zy1);
%     results = [results; {['Gaussian σ=', num2str(sigma)], psnr_val, ssim_val}];
% end
% results
% 
% 
% for ratio = clip_ratios
%     y_attacked = min(max(Y1, quantile(Y1, ratio)), quantile(Y1, 1 - ratio));  % 削波密文
%     recovered_image = UserASPL(y_attacked, Phi, param, num_levels,R3);
%     errorx=sum(sum(abs(recovered_image - zy1).^2));   %MSE误差
%     psnr_val=10*log10(255*255/(errorx/high/width))   %  PSNR
%     ssim_val = ssim(recovered_image, zy1);
%     results = [results; {['Clipping ', num2str(ratio*100), '%'], psnr_val, ssim_val}];
% end
% results
...
[mssim, ssim_map] = ssim(recovered_image,zy1);
mssim
