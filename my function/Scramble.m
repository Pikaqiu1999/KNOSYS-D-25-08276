...
%size_images = row;
 ...
 vec_reshape_image = reshape(original_image, row * col, 1);        %    图像列处理化
 
 enc_vec_reshape_image = zeros(row * col, 1);            %    加密后图像
 ...
 encrypted_image = reshape(enc_vec_reshape_image, row, col); 